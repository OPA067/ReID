...
if __name__ == '__main__':
    ...
    logger = setup_logger('ReID', save_dir=args.output_dir, if_train=args.training, distributed_rank=get_rank())
    ...
    # init training and testing datasets, and model
    train_loader, test_loader = build_dataloader(args)
    model = build_model(args)
    logger.info('total params: %2.fM' % (sum(p.numel() for p in model.parameters()) / (1024 * 2024)))
    model.to(device)
