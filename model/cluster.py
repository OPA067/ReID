...
class CTM(nn.Module):
    ...
    def forward(self, token_dict):
        ...
        # cluster numbers, after = sample_ratio * before
...
if __name__ == '__main__':
    B, N, D = 32, 12, 512
    ctm = CTM(sample_ratio=0.5, embed_dim=512, dim_out=512, k=3)
    block = TCBlock(dim=512, num_heads=8)

    x = torch.randn((B, N, D))
    idx_token = torch.arange(x.size(1))[None, :].repeat(x.size(0), 1)
    agg_weight = x.new_ones(x.size(0), x.size(1), 1)
    ...
    ctm1 = CTM(sample_ratio=0.125, embed_dim=512, dim_out=512, k=3)
    block1 = TCBlock(dim=512, num_heads=16)
